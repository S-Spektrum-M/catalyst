...
std::expected<std::unordered_set<fs::path>, std::string> buildSourceSet(std::vector<std::string> source_dirs,
    ...
    for (const auto &dir : paths) {
        catalyst::logger.log(LogLevel::DEBUG, "Processing source directory: {}", dir.string());
        ...
        if (fs::exists(ignore_file)) {
            catalyst::logger.log(LogLevel::DEBUG, "Found .catalystignore file in: {}", dir.string());
            ...
            for (const auto &profile : profiles) {
                if (ignore_config[profile]) {
                    for (const auto &ignore_pattern : ignore_config[profile]) {
                        ...
                        catalyst::logger.log(LogLevel::DEBUG, "Ignoring pattern: {}", pattern);
            ...
            if (entry.is_regular_file()) {
                ...
                for (const auto &pattern : ignored_files) {
                    ...
                    if (std::regex_match(entry.path().filename().string(), ignore_regex)) {
                        catalyst::logger.log(LogLevel::DEBUG, "Ignoring file: {}", entry.path().string());
                        ignored = true;
                        break;
                    }
                }
