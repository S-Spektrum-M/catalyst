...
std::expected<find_res, std::string> find_git(const std::string &build_dir, const YAML::Node &dep) {
    ...
    catalyst::logger.log(LogLevel::INFO, "Resolving git dependency: {}", dep_name);
    ...
    catalyst::logger.log(LogLevel::INFO, "Composing profiles for git dependency.");
    ...
    if (auto includes = profile["manifest"]["dirs"]["include"]; includes && includes.IsSequence()) {
        for (const auto &dir : includes.as<std::vector<std::string>>()) {
            ...
            catalyst::logger.log(LogLevel::INFO, "Adding include path: {}", abs_include_path.string());
    ...
    if (auto dep_name_node = profile["manifest"]["name"]) {
        ...
        catalyst::logger.log(LogLevel::INFO, "Adding library: {}", lib_name);
